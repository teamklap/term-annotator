...
const showThumbnail = require('../utils/show-thumbnail');
...
/**
 *
 * @param {String} id - id of an element on html to be changed
 * @param {String} className  - name of a class to be updated
 */
const nextThumbnail = function (id, className) { // eslint-disable-line no-unused-vars
	changeStatus('import');
	ipcRenderer.send('get-info', {id, className});
};

/**
 * @param {String} gif gif filename
 * @returns gifFilename saved into .thumbnail file
 */
const saveThumbnailFileName = function (gif, classDir) {
	console.log(classDir);
	ipcRenderer.send('save-thumbnail-filename', {gif, classDir});
};

ipcRenderer.on('next-thumbnail', (event, arg) => {
	const {id} = arg;
	const randomGifId = Math.floor(Math.random() * arg.classInfo.filenames.length);
	const gif = arg.classInfo.filenames[randomGifId];
	saveThumbnailFileName(gif, arg.classInfo.dir);
	showThumbnail(gif, id);
	changeStatus('annotation');
});
...
ipcRenderer.on('all-info', (event, arg) => {
	...
	classNames.forEach((cN, index) => {
		if (classes[cN].numFiles > 0) {
			const id = `im-${index}`;
			if (classes[cN].thumbnail) {
				showThumbnail(classes[cN].thumbnail, id);
			} else {
				const gif = classes[cN].filenames[0];
				saveThumbnailFileName(gif, classes[cN].dir);
				showThumbnail(gif, id);
			}
		}
	});
