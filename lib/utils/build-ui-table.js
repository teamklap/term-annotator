...
module.exports = function ({ numSrcFiles, mainCatsInfo, subCatsInfo }) {
	...
	classNames.forEach((cN, index) => {
		...
		if (mainCatsInfo[cN].numFiles > 0) {
			thumbnailsRow += `<td > \
			<div id="im-${index}"></div> \
			<a href="#"  id="im-${index}" onclick="nextThumbnail(\'' + 'im-${index}'  +'\',\'' + '${cN}' + '\')" class="next">Next Image &raquo;</a> \
			</td>`;
		} else {
		...
		if (subCatsInfo[pN].sub[0]) {
			...
			subCatsInfo[pN].sub[0].choices.forEach((choice, index) => {
				...
				if (choice.numFiles > 0) {
					...
					<div id="im-${index}"></div> \
					<a href="#"  id="im-${index}" onclick="nextThumbnail(\'' + 'im-${index}'  +'\',\'' + '${choice}' + '\')" class="next">Next Image &raquo;</a> \
					</td>`;
				} else {
