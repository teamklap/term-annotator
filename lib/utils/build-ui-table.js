...
module.exports = function ({numSrcFiles, mainCatsInfo, subCatsInfo}) {
	...
	classNames.forEach((cN, index) => {
		...
		const descriptionCln = (mainCatsInfo[cN].description) ? `<td>${mainCatsInfo[cN].description}</td>` : '<td></td>';
		...
		if (mainCatsInfo[cN].numFiles > 0) {
			thumbnailsRow += `<td > \
			<div id="im-${index}"></div> \
			<a href="#"  id="im-${index}" onclick="nextThumbnail(\'' + 'im-${index}'  +'\',\'' + '${cN}' + '\')" class="next">Next Image &raquo;</a></td>`; // eslint-disable-line  no-useless-escape
		} else {
	...
	paramsNames.forEach(pN => {
		if (subCatsInfo[pN].sub[0]) {
			...
			subCatsInfo[pN].sub[0].choices.forEach((choice, index) => {
				...
				if (choice.numFiles > 0) {
					choiceThumbnailsRow += `<td > \
					<div id="im-${index}"></div> \
					<a href="#"  id="im-${index}" onclick="nextThumbnail(\'' + 'im-${index}'  +'\',\'' + '${choice}' + '\')" class="next">Next Image &raquo;</a></td>`; // eslint-disable-line  no-useless-escape
				} else {
