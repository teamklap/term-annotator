			...
			classNames.map(cN => {
				return getDirnames(config.classes[cN], destFolder).then(dirs => {
					return Promise.all(dirs.map(() => {
						...
						const dir = destFolder + '/' + config.classes[cN].dir;
						const thumbnail = fs.existsSync(dir + '/.thumbnail') ? JSON.parse(fs.readFileSync(dir + '/.thumbnail')) : null;
						perClass[cN] = {
							filenames,
						...
						if (config.classes[cN].params) {
							config.classes[cN].params.forEach((param, index) => {
								if (index === 0 && config.classes[cN].params.length > 1) {
									...
									const choices = [];
									const dir = filenames[0].replace(/\/[-._a-z0-9]+.gif$/, '');
									filenames.forEach((f, ind) => {
										const numFiles = f.length;
										const thumbnail = fs.existsSync(dir + '/.thumbnail') ? JSON.parse(fs.readFileSync(dir + '/.thumbnail')) : null;
