...
module.exports = function (config) {
	...
	*/
	...
	ipcMain.on('get-info', (event, arg) => {
		console.log('server, get-info');
		const {id, className} = arg;
		return getAllInfo(config, queue).then(r => {
			event.sender.send('next-thumbnail', {id, classInfo: r.classes[className]});
		}, err => {
			console.log(err);
			event.sender.send('error', err);
		});
	...
	ipcMain.on('save-thumbnail-filename', (event, arg) => {
		const thumbnailGif = arg.gif;
		const thumbnailPath = arg.classDir;
		fs.writeFileSync(thumbnailPath + '/.thumbnail', JSON.stringify(thumbnailGif));
	});
};
